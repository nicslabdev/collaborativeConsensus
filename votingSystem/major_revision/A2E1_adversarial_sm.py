...
files = ['Canada1_clean_WithAnomalies_a2', 'Germany_clean_WithAnomalies_a2', 'Portugal_clean_WithAnomalies_a2',
         'US_Alabama_clean_WithAnomalies_a2', 'vancover_clean_WithAnomalies_a2' ]

boulderData = pd.read_csv("./data/Portugal_clean_WithAnomalies_a2.csv")
dundeeData = pd.read_csv("./data/Germany_clean_WithAnomalies_a2.csv")
netherlandsData = pd.read_csv("./data/vancover_clean_WithAnomalies_a2.csv")
paloaltoData = pd.read_csv("./data/Canada1_clean_WithAnomalies_a2.csv")
perthData = pd.read_csv("./data/US_Alabama_clean_WithAnomalies_a2.csv")

boulderData['Timestamp'] = pd.to_datetime(boulderData['Timestamp'], errors='coerce')
boulderData = boulderData.sort_values(by='Timestamp', ascending=True)

dundeeData['Timestamp'] = pd.to_datetime(dundeeData['Timestamp'], errors='coerce')
dundeeData = dundeeData.sort_values(by='Timestamp', ascending=True)

netherlandsData['Timestamp'] = pd.to_datetime(netherlandsData['Timestamp'], errors='coerce')
netherlandsData = netherlandsData.sort_values(by='Timestamp', ascending=True)

paloaltoData['Timestamp'] = pd.to_datetime(paloaltoData['Timestamp'], errors='coerce')
paloaltoData = paloaltoData.sort_values(by='Timestamp', ascending=True)

perthData['Timestamp'] = pd.to_datetime(perthData['Timestamp'], errors='coerce')
perthData = perthData.sort_values(by='Timestamp', ascending=True)
...
boulderADS = CatboostADS(window_boulder_size, x_boulder_init, y_boulder_init, "BoulderADS_household_A2E1")
...
dundeeADS = LGBMachineADS(window_dundee_size, x_dundee_init, y_dundee_init, "DundeeADS_household_A2E1")
...
paloaltoADS = RandomForestADS(window_paloalto_size, x_paloalto_init, y_paloalto_init, "PaloAltoADS_household_A2E1")
...
perthADS = XGBoostADS(window_perth_size, x_perth_init, y_perth_init, "PerthADS_household_A2E1")
...
for file in files:
    ...
    for eps in epsilon:
        ...
        # Coord 1: ADS metrics
            ...
            graph_folder = f"./graphs/A2E1/A2E1_{file.split('_')[0]}"
