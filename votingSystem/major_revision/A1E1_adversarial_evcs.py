...
#files = ['Boulder_Clean_With_Anomalies.xlsx', 'Dundee_Clean_With_Anomalies_All.xlsx', 'Netherlands_Clean_With_Anomalies.xlsx',
#         'PaloAlto_2018_2022_Clean_With_Anomalies.xlsx', 'Perth_Clean_With_Anomalies.xlsx', ]

files = ['Boulder_Clean_With_Anomalies.xlsx', 'PaloAlto_2018_2022_Clean_With_Anomalies.xlsx', 'Perth_Clean_With_Anomalies.xlsx', ]
...
for idx_file, file in enumerate(files):
    ...
    data = pd.read_excel(f"./data/{file}")
    ...
    boulderADS = CatboostADS(window_boulder_size, x_boulder_init, y_boulder_init, f"{file.split('_')[0]}_1_gb_A1E1")
    ...
    #dundeeADS = LGBMachineADS(window_dundee_size, x_dundee_init, y_dundee_init, f"{file.split('_')[0]}_2_gb_A1E1")
    ...
    paloaltoADS = RandomForestADS(window_paloalto_size, x_paloalto_init, y_paloalto_init, f"{file.split('_')[0]}_4_gb_A1E1")
    ...
    perthADS = XGBoostADS(window_perth_size, x_perth_init, y_perth_init, f"{file.split('_')[0]}_5_gb_A1E1")

    #adsModels = [boulderADS, dundeeADS, netherlandsADS, paloaltoADS, perthADS]
    adsModels = [boulderADS, paloaltoADS, perthADS]
    ...
    for idx_eps, eps in enumerate(epsilon):
        ...
        if eps == 0:
            ...
            attacks = ["all"] #["random", "weak", "strong", "weak_strong", "all", "three_random"]
        for att_idx, attack in enumerate(attacks):
            ...
            for i in range(0, num_splits):
                #if i in [5, 15, 35, 45]:
                #    save_path = f"./Data_distribution/#folder#/analysis_#sample#_#model#_eps{eps}_attack_{att_idx}_split_{i}.png"
                #else:
                #    save_path = None
                if eps == 0: # No attack
                    ...
                    coordinator1_eir_votes.append(round(1 - (float(coordinator1.recall_votes) / coord_1_recall[i]),2))
                    models_coord1_eir_votes.append([round(1 - (now / orig), 2) for now, orig in zip(coordinator1.get_recall_votes_models(), models_coord1_recall[i])])
                    coordinator3_eir_votes.append(round(1 - (float(coordinator3.recall_votes) / coord_3_recall[i]),2))
                    models_coord3_eir_votes.append([round(1 - (now / orig), 2)  for now, orig in zip(coordinator3.get_recall_votes_models(), models_coord3_recall[i])])
                    coordinator5_eir_votes.append(round(1 - (float(coordinator5.recall_votes) / coord_5_recall[i]),2))
                    models_coord5_eir_votes.append([round(1 - (now / orig), 2)  for now, orig in zip(coordinator5.get_recall_votes_models(), models_coord5_recall[i])])
            ...
            for idx, mean_c1_recall_model in enumerate(c1_recall_mean_models):
                ...
                if eps == 0:
                    ...
                    df_results.loc[(attack, "Mean", model_name, "EIR"), eps] = 1 - (mean_c1_recall_model / df_results.loc[("No attack", "Mean", model_name, "Recall"), 0.0])
            ...
            for idx, mean_c3_recall_model in enumerate(c3_recall_mean_models):
                ...
                if eps == 0:
                    ...
                    df_results.loc[(attack, "WeightedMean", model_name, "EIR"), eps] = 1 - (mean_c3_recall_model / df_results.loc[("No attack", "WeightedMean", model_name, "Recall"), 0.0])
            ...
            for idx, mean_c5_recall_model in enumerate(c5_recall_mean_models):
                ...
                if eps == 0:
                    ...
                    df_results.loc[(attack, "Mode", model_name, "EIR"), eps] = 1 - (mean_c5_recall_model / df_results.loc[("No attack", "Mode", model_name, "Recall"), 0.0])
...
#
            ...
            plot(f"DR UC1$_{chr(ord('a') + idx_file)}$ - eps: {eps}", "WeightedMean", coordinator3_recall_votes, models_coord3_recall_votes, f"{graph_folder}/A1E1_Attack[{att_idx}]_{attack}_eps_0.{str(eps).split('.')[1]}_recall_plot_coord3.pdf", "DR", lower_normal, upper_normal)
